...
def main():
    ...
    equation = scene.text.equation("integral_a^b f(x) d x = F(b) - F(a)").fill(WHITE).at(0, 100)
    circle = scene.geometry.circle(120).stroke(GOLD, 6).no_fill().at(-250, -150)
    rect = scene.geometry.rect(200, 120).fill(BLUE).at(250, -150)
