...
scene.play([scene.text("Fondo sólido", role="title").fill(WHITE).write(0.6)])
scene.wait(0.4)
...
scene.play([scene.text("Fondo degradado", role="title").fill(WHITE).write(0.6)])
scene.wait(0.4)

# Omitting background returns to the Scene-level background.
scene.segment("Scene default", Transition.cross_fade(0.35))
scene.play([scene.text("Fondo de Scene", role="title").fill(WHITE).write(0.6)])
scene.wait(0.4)
