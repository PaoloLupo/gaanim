...
curve = scene.geometry.arc(0, 0, 3, 0.0, math.pi * 1.35).no_fill().stroke(BLUE, 0.075)
tracker = scene.viz.parameter(0.0)
dot = scene.geometry.point_on_curve(curve, tracker).fill(GOLD)
tangent = scene.geometry.tangent_on_curve(curve, tracker, length=1.5).stroke(BLACK, 0.075)
title = scene.text("Bézier curve bindings").fill(BLACK).move_to(0, 3.5625, anchor=Anchor.CENTER)
