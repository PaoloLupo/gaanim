...
guide_label = (
    ...
    .fill(GRAY)
    .at(-555, baseline_y + 105, anchor=TextAnchor.BASELINE_LEFT)
)

fade_from = scene.text("HAPPY", size=72).fill(BLUE).at(-390, baseline_y)
fade_to = scene.text("gyp", size=54).fill(GOLD).at(-390, baseline_y)
...
multiline = (
    ...
    .fill(WHITE)
    .at(-555, multiline_y, anchor=TextAnchor.BASELINE_LEFT)
)
multiline_label = (
    scene.text("explicit TextAnchor uses the first line", size=22)
    .fill(GRAY)
    .at(-555, multiline_y - 100, anchor=TextAnchor.BASELINE_LEFT)
)

scene.play(
    [
        fade_from.fade_in(0.5),
        morph_from.fade_in(0.5),
        equation_from.fade_in(0.5),
    ]
)
scene.wait(0.25)
scene.play(
    [
        fade_from.fade_out(0.8),
        fade_to.fade_in(0.8),
        morph_from.morph_to(morph_to, duration=0.8),
        equation_from.step_to(equation_to, duration=0.8),
    ]
)
scene.wait(0.4)
