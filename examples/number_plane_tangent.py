...
def main():
    ...
    axes = [
        scene.line(-700, 0, 700, 0).stroke(WHITE, 2),
        scene.line(0, -360, 0, 360).stroke(WHITE, 2),
    ]
    points = [(x, 160 * math.sin(x / 130) * math.exp(-x / 1200)) for x in range(-620, 621, 40)]
    curve = [
        scene.line(x1, y1, x2, y2).stroke(BLUE, 4)
        for (x1, y1), (x2, y2) in zip(points, points[1:])
    ]
    tangent = scene.line(-120, -90, 220, 165).stroke(YELLOW, 4)
    dot = scene.dot(12).fill(RED).at(50, 60)
