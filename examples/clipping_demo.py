...
orb_mask = scene.geometry.circle(118).no_fill().no_stroke().at(-330, 5)
...
scene.geometry.circle(118).no_fill().stroke("#7DCFFF", 4).at(-330, 5)
...
for index, color in enumerate(("#7AA2F7", "#BB9AF7", "#F7768E", "#E0AF68")):
    stripes.append(
        scene.geometry.rect(105, 300).fill(color).no_stroke().at(130 + index * 105, 5)
    )
stripe_group = scene.geometry.group(stripes)
panel_mask = scene.geometry.rounded_rect(390, 235, 42).no_fill().no_stroke().at(287, 5)
stripe_group.clip(panel_mask)
scene.geometry.rounded_rect(390, 235, 42).no_fill().stroke("#A9B1D6", 4).at(287, 5)
scene.text("Group mask").at(287, -155, anchor=Anchor.CENTER)

# A vector silhouette can be filled by a normalized, animatable level.
drop = scene.geometry.circle(72).no_fill().stroke("#A9B1D6", 4).at(0, -235).opacity(0)
