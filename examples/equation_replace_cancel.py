...
title = scene.text("Resolver una ecuación por términos", role="title").fill(WHITE).move_to(0, 2.875, anchor=Anchor.CENTER)

before = scene.text.equation(
    part("variable", "x"), "+", part("constant", "3"), "=", part("result", "7")
).move_to(0, 0.5, anchor=Anchor.CENTER)
...
caption = scene.text("Cancelamos el término que se elimina").fill(GRAY).move_to(0, -1.875, anchor=Anchor.CENTER)
...
caption = scene.text("Reemplazamos el paso y conservamos x").fill(GRAY).move_to(0, -1.875, anchor=Anchor.CENTER)
