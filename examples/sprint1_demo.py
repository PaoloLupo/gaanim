...
def main():
    ...
    title = scene.text("Math and annotations", role="title").fill(WHITE).at(0, 260)
    ...
    formula = scene.text.equation("r = sqrt(x^2 + y^2)").fill(GOLD).at(-170, -170)
    callout = scene.geometry.rounded_rect(240, 80, 12).stroke(BLUE, 3).no_fill().at(260, -170)
    callout_text = scene.text("distance from origin").fill(YELLOW).at(260, -170)
    point = scene.geometry.dot(12).fill(RED).at(180, 140)
