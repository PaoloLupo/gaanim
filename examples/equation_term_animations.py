...
title = scene.text("Ecuaciones por significado, no por glifos", role="title").fill(WHITE).move_to(0, 3.375)
...
caption = scene.text("1. Escribir cada término semántico").fill(GRAY).move_to(0, -1.25)
...
caption = scene.text("2. Revelar c² desde abajo").fill(GRAY).move_to(0, -1.25)
...
scene.play([caption.animate.fade_out().duration(0.2)])
caption = scene.text("3. Indicar la masa").fill(GRAY).move_to(0, -1.25)
scene.play([caption.animate.fade_in()])
...
scene.play([caption.animate.fade_out().duration(0.2)])
caption = scene.text("4. Enfocar los términos que intervienen").fill(GRAY).move_to(0, -1.25)
scene.play([caption.animate.fade_in()])
