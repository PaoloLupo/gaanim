...
title = scene.text("Reactive curved arrow", role="title").fill(BLACK).at(0, 220, anchor=Anchor.CENTER)

disk = scene.geometry.circle(155).no_fill().stroke(BLACK, 4)
marker = scene.geometry.dot(11).fill(GOLD).at(155, 0)
...
label = scene.text("theta").fill(BLACK).at(20, 45, anchor=Anchor.CENTER)
