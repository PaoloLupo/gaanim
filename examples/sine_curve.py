...
circle_radius = 100.0
circle = (
    scene.circle(circle_radius).at(origin_x, origin_y).stroke(WHITE, 2.0).no_fill()
)

# --- Orbiting dot ---
dot = scene.dot(8).fill(Color(250, 250, 210)).at(origin_x + circle_radius, origin_y)
dot.add_updater(
    Updater.orbit(
        cx=origin_x,
        cy=origin_y,
...
proj_dot = scene.dot(5).fill(Color(255, 200, 100)).at(-300.0, origin_y)
