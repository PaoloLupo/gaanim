...
- Brand assets in `docs/assets/brand/` are the exception: they are checked in and
  generated by `python tools/generate_brand.py` (guidelines in that folder's
  `README.md`). Change the script, not the SVG/PNG/ICO files. The docs home hero
  animation is `docs/assets/hero.js`.
