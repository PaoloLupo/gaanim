    ...
    egui::Area::new("overlays_top_bar".into())
        ...
        .show(ctx, |ui| {
            egui::Frame::new()
                ...
                .stroke(egui::Stroke::new(1.0, egui::Color32::from_rgba_premultiplied(65, 65, 80, 140)))
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ui.label(
                        ...
                        let bounds_btn = egui::Button::new(
                            egui::RichText::new(if overlays.show_bounds { "⬜ Límites ✓" } else { "⬜ Límites" })
                                .size(11.0)
                        ...
                        if ui.add(bounds_btn).on_hover_text("B: alternar límites del canvas").clicked() {
                            overlays.show_bounds = !overlays.show_bounds;
                        }
                        let coords_btn = egui::Button::new(
                            egui::RichText::new(if overlays.show_coords { "⊕ Coords ✓" } else { "⊕ Coords" })
                                .size(11.0)
                        ...
                        .corner_radius(3.0)
                        .fill(egui::Color32::from_rgba_premultiplied(30, 30, 45, 140));
                        if ui.add(coords_btn).on_hover_text("C: alternar ejes y coordenadas del cursor").clicked() {
                            overlays.show_coords = !overlays.show_coords;
                        }
                        let grid_btn = egui::Button::new(
                            egui::RichText::new(if overlays.show_grid { "▦ Grilla ✓" } else { "▦ Grilla" })
                                .size(11.0)
                        ...
                        .corner_radius(3.0)
                        .fill(egui::Color32::from_rgba_premultiplied(30, 30, 45, 140));
                        if ui.add(grid_btn).on_hover_text("G: alternar grilla cada 100u").clicked() {
...
pub fn scene_overlays_system(
    ...
    let (bmin, bmax, label) = if let Some(bg) = canvas_bg.as_ref() {
        ...
        (glam::DVec2::new(b.min.x, b.min.y), glam::DVec2::new(b.max.x, b.max.y), label)
    } else {
    ...
    let corners_screen: Vec<egui::Pos2> =
        corners_world.iter().map(|w| world_to_egui(&cam, window, *w)).collect();
    ...
    egui::Area::new("scene_overlays".into())
        ...
        .show(ctx, |ui| {
            ...
            if overlays.show_bounds {
                ...
                for &c in &corners_screen {
                    ...
                    painter.line_segment(
                        [egui::pos2(c.x - corner_len, c.y), egui::pos2(c.x + 4.0, c.y)],
                        stroke,
                    );
                    painter.line_segment(
                        [egui::pos2(c.x, c.y - corner_len), egui::pos2(c.x, c.y + 4.0)],
                        stroke,
                    );
                }
                ...
                let top_center = egui::pos2(
                    ...
                    corners_screen[0].y.min(corners_screen[1].y).min(corners_screen[2].y).min(corners_screen[3].y),
                );
                ...
                let text_pos = egui::pos2(top_center.x - galley.size().x * 0.5, top_center.y - galley.size().y - 4.0);
                let bg_rect = egui::Rect::from_min_size(
                    text_pos - egui::vec2(4.0, 1.0),
                    galley.size() + egui::vec2(8.0, 2.0),
                );
                painter.rect_filled(bg_rect, 3.0, egui::Color32::from_rgba_premultiplied(18, 18, 22, 200));
            ...
            if overlays.show_coords {
                // Ejes X/Y si el origen está dentro del canvas
                let origin_visible = bmin.x <= 0.0
                    && bmax.x >= 0.0
                    && bmin.y <= 0.0
                    && bmax.y >= 0.0;
                if origin_visible {
                    ...
                    painter.circle_stroke(origin, 3.0, egui::Stroke::new(1.0, egui::Color32::from_rgb(60, 60, 70)));
                    ...
                    // Punta X
                    painter.line_segment(
                        [x_end, x_end - egui::vec2(dir_x.x - dir_x.y * 0.4, dir_x.y + dir_x.x * 0.4)],
                        x_stroke,
                    );
                    painter.line_segment(
                        [x_end, x_end - egui::vec2(dir_x.x + dir_x.y * 0.4, dir_x.y - dir_x.x * 0.4)],
                    ...
                    // Punta Y
                    painter.line_segment(
                        [y_end, y_end - egui::vec2(dir_y.x - dir_y.y * 0.4, dir_y.y + dir_y.x * 0.4)],
                        y_stroke,
                    );
                    painter.line_segment(
                        [y_end, y_end - egui::vec2(dir_y.x + dir_y.y * 0.4, dir_y.y - dir_y.x * 0.4)],
