...
pub enum ExportQuality { Draft, Standard, Production }

impl ExportQuality {
    fn fps(self) -> u32 { match self { Self::Draft => 30, _ => 60 } }
    fn label(self) -> &'static str { match self { Self::Draft => "Draft", Self::Standard => "Standard", Self::Production => "Production" } }
    fn crf(self) -> u32 { match self { Self::Draft => 24, Self::Standard => 18, Self::Production => 14 } }
    fn encoding_speed(self) -> EncodingSpeed {
        match self { Self::Draft => EncodingSpeed::Fast, Self::Standard => EncodingSpeed::Balanced, Self::Production => EncodingSpeed::Best }
    }
}
...
impl Default for ExportState {
    fn default() -> Self {
        Self {
            dialog_open: false, format: ExportFormat::Mp4, quality: ExportQuality::Standard,
            output_path: "output.mp4".to_string(), active: false, show_complete: false,
            message: String::new(), progress_shared: Arc::new(Mutex::new(None)),
        }
    }
}
...
pub fn export_dialog_system(
    ...
    if state.show_complete {
        egui::Window::new("Export Complete")
            ...
            .show(ctx, |ui| {
                ...
                ui.add_space(8.0);
                if ui.button("OK").clicked() { trigger_ok = true; }
            });
    }

    if trigger_ok { state.show_complete = false; }

    if state.active {
        let (prog_frame, prog_total, prog_done) = {
            let lock = state.progress_shared.lock().unwrap();
            if let Some(ref prog) = *lock {
                (prog.current_frame, prog.total_frames, prog.current_frame >= prog.total_frames)
            } else {
                (0, 1, false)
            }
        };
        ...
        if prog_done {
            ...
            *state.progress_shared.lock().unwrap() = None;
            ...
            let progress = if prog_total > 0 { prog_frame as f32 / prog_total as f32 } else { 0.0 };
            ...
            if trigger_cancel { state.active = false; *state.progress_shared.lock().unwrap() = None; }
            return;
        }
    }
    ...
    egui::Window::new("Export Scene")
        ...
        .show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("Quality:");
                egui::ComboBox::from_id_salt("export_qual").selected_text(current_quality.label())
                    .show_ui(ui, |ui| {
                        ui.selectable_value(&mut current_quality, ExportQuality::Draft, "Draft (480p30)");
                        ui.selectable_value(&mut current_quality, ExportQuality::Standard, "Standard (1080p60)");
                        ui.selectable_value(&mut current_quality, ExportQuality::Production, "Production (4K60)");
                    });
            });
            ui.horizontal(|ui| {
                ui.label("Output:");
            ...
            ui.label(format!("Duration: {:.1}s → {} frames at {}fps", dur, total, fps));
            ui.add_space(8.0);
            ui.horizontal(|ui| {
                if ui.button("Export").clicked() { trigger_export = true; }
                if ui.button("Cancel").clicked() { trigger_cancel = true; }
            });
        });
    ...
    if trigger_export {
        if !has_replay {
            ...
            *progress.lock().unwrap() = Some(ExportProgress {
                current_frame: 0, total_frames: total,
