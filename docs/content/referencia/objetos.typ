...
#api-entry(
  ...
  desc: [Transitions, causality, flow. For curved, use `curved_arrow`.
    Dimensions are in scene units. Omitted values use head length 0.18,
    head width 0.15 and body width 0.036. `max_head_ratio` in `(0, 1]` caps head
    ...
    Add `max_head_ratio=0.3` for short arrows and `.no_stroke()` for a solid silhouette.],
)[
```python
# show-code: true
from gaanim import BLUE, GOLD, WHITE, RED, GREEN, Scene
...
#api-entry(
  ...
  desc: [Bidirectional relations, spans. Omitted head metrics use 0.18 by 0.15 scene units, with a 0.036 body; heads shrink so they never overlap.],
)[
```python
# show-code: true
from gaanim import BLUE, GOLD, WHITE, RED, GREEN, Scene
...
from gaanim import BLUE, GOLD, WHITE, RED, GREEN, Scene
scene = Scene(frame=(16, 9), background="#0f172a")
arr = scene.geometry.curved_arrow_arc(0, 0, 2.5, 0.2, 1.8).fill(GOLD)
...
#api-entry(
  ...
  desc: [Engineering drawings. For reactive, use `dimension_between`. Uses 0.02-unit extension lines and the default `double_arrow` heads.],
)[
```python
# show-code: true
from gaanim import BLUE, GOLD, WHITE, RED, GREEN, Scene
...
See #link("/referencia/visualization/", "Visualization API") for Cartesian,
...
#link("/referencia/text/", "Text — content, math, style, flow, selections, Layout, and animation").
...
#api-entry(
  ...
  kind: "factory",
  signature: "lottie(path: str, *, animation_id?, theme_id?, state_machine_id?, width?, height?, fit=\"contain\", offset=0.0, duration?, loop=false, speed=1.0) -> Lottie",
  params: ((name: "path", type: "str", default: none, desc: [Archivo Lottie JSON o paquete dotLottie v1/v2 local.]), (name: "animation_id", type: "str | None", default: "None", desc: [Animación del paquete; excluye state_machine_id.]), (name: "theme_id", type: "str | None", default: "None", desc: [Tema estático inicial; por defecto usa initialTheme si existe.]), (name: "state_machine_id", type: "str | None", default: "None", desc: [Máquina de estados del paquete; se activa sin alargar el timeline.]), (name: "fit", type: "str", default: "\"contain\"", desc: ["contain|cover|stretch"]), (name: "offset", type: "float", default: "0.0", desc: [Inicio dentro de la composición, en segundos.]), (name: "duration", type: "float", default: "hasta el final", desc: [Duración seleccionada de la fuente.]), (name: "loop", type: "bool", default: "false", desc: [Repite el intervalo seleccionado.]), (name: "speed", type: "float", default: "1.0", desc: [Velocidad positiva.]),),
  returns: (type: "Lottie", desc: [Declaración transformable que se activa con `Scene.play`.]),
  desc: [Los selectores opcionales animation_id, theme_id y state_machine_id eligen contenido del paquete. Animación y máquina son mutuamente excluyentes. Una máquina se controla con set_input, fire_event y set_theme en el cursor; activarla no alarga el batch. Renderiza Lottie JSON y dotLottie con Velato dentro de la misma escena Vello. Declarar no inicia la animación: `scene.play([clip])` fija su inicio en el timeline. Un clip finito aporta su duración al batch; un loop no lo alarga. `clip.warnings` enumera características detectadas que Velato omite o aproxima.],
)[
```python
# show-code: true
from gaanim import Scene
...
#api-entry(
  ...
  kind: "factory",
  signature: "rolling_number(value=0.0, *, decimals=0, min_digits=1, group_separator='', decimal_separator='.', prefix='', suffix='', show_plus=False, font_family=None, weight=None, font_size=0.75, digit_spacing=0.02, line_height=1.25, mode='odometer', direction='up', color=None) -> RollingNumber",
  params: (
...
habitual de Gaanim. La familia y `weight` se resuelven como en `scene.text`: por
el nombre de familia guardado en cada fuente, incluidas las registradas con
`Theme(font_files=...)` y las fuentes variables. Opciones inválidas,
