...
#api-entry(
  ...
  desc: [Usa FFmpeg/ffprobe. Declarar no inicia frames ni audio: `scene.play([clip])` fija el inicio absoluto. Un video finito sin loop aporta su duración de salida al batch; un loop no alarga el timeline. `width`, `height`, `fit` y `crop` tienen la misma semántica que `Scene.image`.],
)[
```python
# show-code: true
from gaanim import Scene
scene = Scene(960, 540)
# Con un MP4 local: clip = scene.video("assets/clip.mp4", width=720, duration=4, loop=True, volume=0.8); scene.play([clip])
