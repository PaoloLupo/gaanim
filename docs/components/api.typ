  ...
  if target() != "bundle" {
    block(
      ...
      [
        ...
        #if params.len() > 0 [
          #v(6pt)
          #text(weight: "bold", size: 9pt, [Parameters:])
          #v(2pt)
          #table(
            ...
            ..params.map(p => (
              ...
              if p.at("default", default: none) == none { text(fill: rgb("#dc2626"), [required]) } else { raw(str(p.at("default"))) },
              p.at("desc", default: [-]),
            )).flatten()
          )
        ]
        #if returns != none [
          #v(4pt)
          #text(weight: "bold", size: 9pt, [Returns: ])
